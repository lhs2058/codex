...
describe("legacy Excel adapters", () => {
  it("parses AOI line and model rows from the anonymized workbook and recomputes NG", async () => {
    ...
    expect(result.rows).toEqual(expect.arrayContaining([
      expect.objectContaining({ sourceSheet: "AOI Line", sourceRow: 8, productionDate: "2026-07-27", lineCode: "LINE-1", modelCode: "MODEL-A", processCode: "AOI", inputQty: 100, okQty: 97, ngQty: 3, timeSlotCode: "A" }),
      expect.objectContaining({ sourceSheet: "aoi model", sourceRow: 8, lineCode: "LINE-2", modelCode: "MODEL-B", inputQty: 50, okQty: 49, ngQty: 1 }),
    ]));
  });
  ...
  it("parses SPI line and model rows without trusting stored yield", async () => {
    ...
    expect(result.rows.map((row) => [row.lineCode, row.modelCode, row.inputQty, row.okQty, row.ngQty])).toEqual([
      ["LINE-1", "MODEL-A", 40, 39, 1], ["LINE-2", "MODEL-B", 30, 27, 3],
    ]);
  });
  ...
  it("keeps daily ICT and Xray quality rows untimed and skips aggregate rows with diagnostics", async () => {
    ...
    expect(ict.rows[0]).toMatchObject({ sourceSheet: "Data HS Công Đoạn ICT", sourceRow: 8, timeSlotCode: null, lineCode: "LINE-1", modelCode: "MODEL-A", inputQty: 20, okQty: 19, ngQty: 1 });
    expect(xray.rows).toHaveLength(1);
    expect(xray.rows[0]).toMatchObject({ sourceSheet: "Xray", sourceRow: 8, timeSlotCode: null, lineCode: "LINE-2", modelCode: "MODEL-B", inputQty: 15, okQty: 14, ngQty: 1 });
    expect([...ict.diagnostics, ...xray.diagnostics]).toEqual(expect.arrayContaining([expect.objectContaining({ code: "missing-required-value", sourceRow: 9 })]));
  });
  ...
  it("expands production A-E cells, retains actuals and downtime, and ignores CAPA as input", async () => {
    ...
    expect(result.rows).toEqual(expect.arrayContaining([
      expect.objectContaining({ sourceSheet: "27.07", sourceRow: 7, productionDate: "2026-07-27", lineCode: "LINE-1", modelCode: "MODEL-A", processCode: "ROUTER", timeSlotCode: "A", inputQty: 0, actualQty: 8, downtimeMinutes: 5, note: "setup" }),
      expect.objectContaining({ sourceSheet: "27.07", sourceRow: 8, lineCode: "LINE-2", modelCode: "MODEL-B", processCode: "XRAY", timeSlotCode: "E", actualQty: 5 }),
      expect.objectContaining({ sourceSheet: "27.07", sourceRow: 9, lineCode: "ROUTER-2", modelCode: "MODEL-A", processCode: "ROUTER", timeSlotCode: "A", actualQty: 2 }),
    ]));
    expect(result.rows.every((row) => row.inputQty === 0 && row.okQty === 0 && row.ngQty === 0)).toBe(true);
    expect(result.diagnostics).toEqual(expect.arrayContaining([expect.objectContaining({ code: "missing-required-value", sourceRow: 10 })]));
  });
  ...
  it("returns row-specific diagnostics instead of throwing for malformed in-memory quality data", () => {
    ...
    expect(result.diagnostics).toContainEqual(expect.objectContaining({ sourceRow: 2, code: "invalid-count", field: "okQty" }));
  });
});
