...
export function AdminPage({ repository, createUser = invokeAdminCreateUser }: { repository?: MasterDataRepository; createUser?: (input: NewUser) => Promise<unknown> }) {
  const [snapshot, setSnapshot] = useState<MasterDataSnapshot | null>(null); const [error, setError] = useState(""); const [mutating, setMutating] = useState(false); const mounted = useRef(true); const generation = useRef(0); const getRepository = () => repository ?? createMasterDataRepository();
  ...
  const mutate = async (action: () => Promise<unknown>) => { if (mutating) return; setMutating(true); setError(""); try { await action(); await refresh(); } catch (cause) { if (mounted.current) setError(cause instanceof Error ? cause.message : "Unable to save changes."); throw cause; } finally { if (mounted.current) setMutating(false); } };
  useEffect(() => { mounted.current = true; void refresh(); return () => { mounted.current = false; generation.current++; }; }, [repository]);
  return <main><h1>Administration</h1><p>Admin workspace</p>{!snapshot && !error && <p role="status">Loading master data…</p>}{error && <p role="alert">{error}</p>}{error && <button onClick={() => void refresh()} disabled={mutating}>Retry</button>}{snapshot && <><MasterDataEditor snapshot={snapshot} createModel={(input) => mutate(() => getRepository().createModel(input))} deactivateDowntimeReason={(id, version) => mutate(() => getRepository().deactivateDowntimeReason(id, version))} /><StandardTimeEditor snapshot={snapshot} save={(input) => mutate(() => getRepository().saveStandardTime(input))} /><UserEditor createUser={createUser} /></>}</main>;
}
